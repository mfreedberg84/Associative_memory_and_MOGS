...
%
% indicate where your data output from h1_acquisition_retention.m went  
...
%   2) acquisition asymptote, magnitude, and rate, and retention
%      are regressed against associative memory performance.
...
%
%   Plots
%   1) Comparison of ENC and SEM memory performance (not shown in paper).
%   1) Figure 3A
%   2) Figure 3B
%   3) Figure 3C
%   4) Figure 3D
...
text(2, 0.55, ['SEM is ' num2str(std(avgMemPerformance_SEM,'omitmissing')/sqrt(14), '%.3f')], 'Color', 'b', 'FontSize', 16);

%% Signficance testing

% ENC
disp('Testing ENC memory performance against zero...')
zrs = zeros(15,1);
zrs(15,1) = 0.25;
disp(['ENC memory performance = ', ...
    num2str(mean(avgMemPerformance_ENC)), ...
    ' SEM = ', ...
    num2str(std(avgMemPerformance_ENC)/sqrt(length(avgMemPerformance_ENC))), ...
    '.'] ...
    )

[~,p,~,stats] = ttest(avgMemPerformance_ENC, zrs,'Alpha',0.05,'Tail','right');

if p < 0.05
    disp('Reject the null hypothesis that ENC group memory performance is equal to zero');
    disp(['t(', num2str(stats.df), ') = ' num2str(stats.tstat), ', p = ' num2str(p) '.']);
else
    disp('Fail to reject the null hypothesis that ENC group memory performance is equal to zero');
    disp(['t(', num2str(stats.df), ') = ' num2str(stats.tstat), ', p = ' num2str(p) '.']);
end
...
% SEM
disp('Testing SEM memory performance against zero...')
zrs(14,1) = 0.25;
disp(['SEM memory performance = ', ...
    num2str(mean(avgMemPerformance_SEM,'omitmissing')), ...
    ' SEM = ', ...
    num2str(std(avgMemPerformance_SEM,'omitmissing')/sqrt(length(avgMemPerformance_SEM)-1)), ...
    '.'] ...
    )

[~,p,~,stats] = ttest(avgMemPerformance_SEM, zrs,'Alpha',0.05,'Tail','right');

if p < 0.05
    disp('Reject the null hypothesis that SEM group memory performance is equal to zero');
    disp(['t(', num2str(stats.df), ') = ' num2str(stats.tstat), ', p = ' num2str(p) '.']);
else
    disp('Fail to reject the null hypothesis that SEM group memory performance is equal to zero');
    disp(['t(', num2str(stats.df), ') = ' num2str(stats.tstat), ', p = ' num2str(p) '.']);
end
...
% ENC vs. SEM
disp('Testing ENC vs. SEM...')
[~,p,~,stats] = ttest2( ...
    avgMemPerformance_ENC, avgMemPerformance_SEM, ...
    'Alpha',0.05, ...
    'Tail','both' ...
    );

if p < 0.05
    disp('Reject the null hypothesis that ENC and SEM memory performance is equal');
    disp(['t(', num2str(stats.df), ') = ' num2str(stats.tstat), ', p = ' num2str(p) '.']);
else
    disp('Fail to reject the null hypothesis that ENC and SEM memory performance is equal');
    disp(['t(', num2str(stats.df), ') = ' num2str(stats.tstat), ', p = ' num2str(p) '.']);
end
...
scatter(enc_corr_a(:,1), enc_corr_a(:,2),150, ...
    'MarkerEdgeColor',[0 0 0], ...
    'MarkerFaceColor',[0 0 1], ...
    'LineWidth',1.5 ...
    ); hold on; 

tlp = polyfit(enc_corr_a(:,1), enc_corr_a(:,2), 1); 
tlpx = [min(enc_corr_a(:,1)) max(enc_corr_a(:,1))]; 
tlpy = polyval(tlp, tlpx); 
plot(tlpx, tlpy, 'LineWidth', 6,'Color',"b"); 
scatter(sem_corr_a(:,1), sem_corr_a(:,2),150, ...
    'MarkerEdgeColor',[0 0 0], ...
    'MarkerFaceColor',[1 0 0], ...
    'LineWidth',1.5 ...
    ); 

tlp = polyfit(sem_corr_a(:,1), sem_corr_a(:,2), 1); 
tlpx = [min(sem_corr_a(:,1)) max(sem_corr_a(:,1))]; 
tlpy = polyval(tlp, tlpx); 
...
n1 = length(enc_corr_a);
n2 = length(sem_corr_a);

z1 = atanh(ENC_ep_a_r(1,2));
z2 = atanh(SEM_ep_a_r(1,2));

% Handle the edge case where r = 1 or r = -1
if isinf(z1)
    z1 = atanh(sign(r1) * (1 - 1e-10));
end
if isinf(z2)
    z2 = atanh(sign(r2) * (1 - 1e-10));
end

% Compute the standard error of the difference
se_diff_z = sqrt(1 / (n1 - 3) + 1 / (n2 - 3));

% Compute the Z-statistic for the difference between the two z-values
z_stat = (z1 - z2) / se_diff_z;

% Calculate the two-tailed p-value
p_value = 1 - normcdf(z_stat);

% --- Display results ---
fprintf('Correlation 1 (n=%d): r = %.4f, z = %.4f\n', n1, ENC_ep_a_r(1,2), z1);
fprintf('Correlation 2 (n=%d): r = %.4f, z = %.4f\n', n2, SEM_ep_a_r(1,2), z2);
fprintf('Z-statistic for difference: %.4f\n', z_stat);
fprintf('p-value: %.4f\n', p_value);

if p_value < 0.05
    disp('Result: The ENC and SEM correlations are significantly different for asymptote');
else
    disp('Result: The ENC and SEM correlations are NOT significantly different for asymptote');
end
...
sem_corr_b(10,:) = [];
scatter(enc_corr_b(:,1), enc_corr_b(:,2),150, ...
    'MarkerEdgeColor',[0 0 0], ...
    'MarkerFaceColor',[0 0 1], ...
    'LineWidth',1.5); 
...
tlp = polyfit(enc_corr_b(:,1), enc_corr_b(:,2), 1); 
tlpx = [min(enc_corr_b(:,1)) max(enc_corr_b(:,1))]; 
tlpy = polyval(tlp, tlpx); 
plot(tlpx, tlpy, 'LineWidth', 6,'Color',"b"); 
scatter(sem_corr_b(:,1), sem_corr_b(:,2),150, ...
    'MarkerEdgeColor',[0 0 0], ...
    'MarkerFaceColor',[1 0 0], ...
    'LineWidth',1.5 ...
    ); 

tlp = polyfit(sem_corr_b(:,1), sem_corr_b(:,2), 1); 
tlpx = [min(sem_corr_b(:,1)) max(sem_corr_b(:,1))]; 
tlpy = polyval(tlp, tlpx); plot(tlpx, tlpy, 'LineWidth', 6,'Color',"r"); 
ylim([0 10]);
xlim([0 1]);
set(gca,'FontSize',16);
...
% Contrasting correlations
n1 = length(enc_corr_b);
n2 = length(sem_corr_b);

z1 = atanh(ENC_ep_b_r(1,2));
z2 = atanh(SEM_ep_b_r(1,2));
...
% --- Display results ---
fprintf('Correlation 1 (n=%d): r = %.4f, z = %.4f\n', n1, ENC_ep_b_r(1,2), z1);
fprintf('Correlation 2 (n=%d): r = %.4f, z = %.4f\n', n2, SEM_ep_b_r(1,2), z2);
fprintf('Z-statistic for difference: %.4f\n', z_stat);
fprintf('p-value: %.4f\n', p_value);

if p_value < 0.05
    disp('Result: The ENC and SEM correlations are significantly different for magnitude');
else
    disp('Result: The ENC and SEM correlations are NOT significantly different for magnitude');
end
...
scatter(enc_corr_c(:,1), enc_corr_c(:,2),150, ...
    'MarkerEdgeColor',[0 0 0], ...
    'MarkerFaceColor',[0 0 1], ...
    'LineWidth',1.5); 
hold on; 
...
tlpy = polyval(tlp, tlpx); 
plot(tlpx, tlpy, 'LineWidth', 6,'Color',"b"); 
scatter(sem_corr_c(:,1), sem_corr_c(:,2),150, ...
    'MarkerEdgeColor',[0 0 0], ...
    'MarkerFaceColor',[1 0 0], ...
    'LineWidth',1.5 ...
    ); 

tlp = polyfit(sem_corr_c(:,1), sem_corr_c(:,2), 1); 
tlpx = [min(sem_corr_c(:,1)) max(sem_corr_c(:,1))]; 
tlpy = polyval(tlp, tlpx); plot(tlpx, tlpy, 'LineWidth', 6,'Color',"r"); 
ylim([0 10]);
xlim([0 1]);
set(gca,'FontSize',24);
...
% Contrasting correlations
n1 = length(enc_corr_c);
n2 = length(sem_corr_c);

z1 = atanh(ENC_ep_c_r(1,2));
z2 = atanh(SEM_ep_c_r(1,2));
...
% --- Display results ---
fprintf('Correlation 1 (n=%d): r = %.4f, z = %.4f\n', n1, ENC_ep_c_r(1,2), z1);
fprintf('Correlation 2 (n=%d): r = %.4f, z = %.4f\n', n2, SEM_ep_c_r(1,2), z2);
fprintf('Z-statistic for difference: %.4f\n', z_stat);
fprintf('p-value: %.4f\n', p_value);

if p_value < 0.05
    disp('Result: The ENC and SEM correlations are significantly different for rate');
else
    disp('Result: The ENC and SEM correlations are NOT significantly different for rate');
end
...
%% Plot associative memory performance by retention
sem_corr_ret(10,:) = [];

scatter(enc_corr_ret(:,1), enc_corr_ret(:,2),150, ...
    'MarkerEdgeColor',[0 0 0], ...
    'MarkerFaceColor',[0 0 1], ...
    'LineWidth',1.5); 
hold on; 

tlp = polyfit(enc_corr_ret(:,1), enc_corr_ret(:,2), 1); 
tlpx = [min(enc_corr_ret(:,1)) max(enc_corr_ret(:,1))]; 
tlpy = polyval(tlp, tlpx); plot(tlpx, tlpy, 'LineWidth', 6,'Color',"b"); 
scatter(sem_corr_ret(:,1), sem_corr_ret(:,2),150, ...
    'MarkerEdgeColor',[0 0 0], ...
    'MarkerFaceColor',[1 0 0], ...
    'LineWidth',1.5 ...
    ); 

tlp = polyfit(sem_corr_ret(:,1), sem_corr_ret(:,2), 1); 
tlpx = [min(sem_corr_ret(:,1)) max(sem_corr_ret(:,1))]; 
tlpy = polyval(tlp, tlpx); plot(tlpx, tlpy, 'LineWidth', 6,'Color',"r"); 
ylim([0 6]);
xlim([0 1]);
set(gca,'FontSize',16);
...
% Contrasting correlations
n1 = length(enc_corr_ret);
n2 = length(sem_corr_ret);

z1 = atanh(ENC_ep_ret_r(1,2));
z2 = atanh(SEM_ep_ret_r(1,2));
...
% --- Display results ---
fprintf('Correlation 1 (n=%d): r = %.4f, z = %.4f\n', n1, ENC_ep_ret_r(1,2), z1);
fprintf('Correlation 2 (n=%d): r = %.4f, z = %.4f\n', n2, SEM_ep_ret_r(1,2), z2);
fprintf('Z-statistic for difference: %.4f\n', z_stat);
fprintf('p-value: %.4f\n', p_value);

if p_value < 0.05
    disp('Result: The ENC and SEM correlations are significantly different for retention');
else
    disp('Result: The ENC and SEM correlations are NOT significantly different for retention');
end
