...
%    figure is not shown in the report but is helpful for identifying the
...
%    to be run by the user in addition to this script. They must 
%    be in the same folder as the h1_acquisition_retention.m script. They are 
%    "h2_curvefit_mogs.m," "h3_exponentialfit.m," "h4_exponential_ls.m," 
%    and "h5_exponential.ms." These functions will determine the
%    acquisition asymptote, magnitude, and rate for all participants using
%    their learning curve described above.
...
sgt = sgtitle('Number of correct sequences by gorup and participant', ...
    'Color','black' ...
    );
...
for grp=1:3 %group
    for trl=1:24 % trial
        ...
        sem_perf(grp,trl) = std( ...
            numb_corr_performance(:,trl,grp), ...
            'omitmissing')/sqrt(size(numb_corr_performance,1) ...
            );
    end
end
...
shadedErrorBar(x(1,:),y(2,:),err(2,:), ...
    'lineprops',{'-bo', ...
    'MarkerFaceColor', ...
    'b','MarkerSize',12} ...
    );

shadedErrorBar(x(1,:),y(3,:),err(3,:), ...
    'lineprops',{'-ro', ...
    'MarkerFaceColor','r', ...
    'MarkerSize',12} ...
    );
...
xlim([1 24]);
xlabel('trial','FontSize',36);
ylabel(['Mean Number of correctly ' newline 'performed sequences'],'FontSize',16)
set(gca,'FontSize',16,'XColor','k','YColor','k');
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 1, 0.96]);
title([' Effect of group on number of correct sequences (acquisition) ' newline]); 
legend('Rest','ENC','SEM','Location','southeast',fontsize=24);
legend('Box','off')
hold off
...
disp('--- acquisition asymptote ---')
a_data = [cf_output(1:15,3) cf_output(16:30,3) cf_output(31:45,3)];

disp(['Rest asymptote: ', ...
    num2str(mean(a_data(:,1),'omitmissing')), ...
    ' ± ', num2str(std(a_data(:,1), ...
    'omitmissing')/sqrt(15))] ...
    )

disp(['ENC asymptote: ', ...
    num2str(mean(a_data(:,2),'omitmissing')), ...
    ' ± ', ...
    num2str(std(a_data(:,2), ...
    'omitmissing')/sqrt(15))] ...
    )

disp(['SEM asymptote: ', ...
    num2str(mean(a_data(:,3),'omitmissing')), ...
    ' ± ', ...
    num2str(std(a_data(:,3), ...
    'omitmissing')/sqrt(14))] ...
    )

disp('--- acquisition magnitude ---')
b_data = [cf_output(1:15,4) cf_output(16:30,4) cf_output(31:45,4)];

disp(['Rest magnitude: ', ...
    num2str(mean(b_data(:,1),'omitmissing')), ...
    ' ± ', num2str(std(b_data(:,1), ...
    'omitmissing')/sqrt(15))] ...
    )

disp(['ENC magnitude: ', ...
    num2str(mean(b_data(:,2),'omitmissing')), ...
    ' ± ', ...
    num2str(std(b_data(:,2), ...
    'omitmissing')/sqrt(15))] ...
    )

disp(['SEM magnitude: ', ...
    num2str(mean(b_data(:,3),'omitmissing')), ...
    ' ± ', ...
    num2str(std(b_data(:,3), ...
    'omitmissing')/sqrt(14))] ...
    )

disp('--- acquisition rate ---')
c_data = [cf_output(1:15,5) cf_output(16:30,5) cf_output(31:45,5)];

disp(['Rest rate: ', ...
    num2str(mean(c_data(:,1),'omitmissing')), ...
    ' ± ', ...
    num2str(std(c_data(:,1), ...
    'omitmissing')/sqrt(15))] ...
    )

disp(['ENC rate: ', ...
    num2str(mean(c_data(:,2),'omitmissing')), ...
    ' ± ', ...
    num2str(std(c_data(:,2), ...
    'omitmissing')/sqrt(15))] ...
    )

disp(['SEM rate: ', ...
    num2str(mean(c_data(:,3),'omitmissing')), ...
    ' ± ', ...
    num2str(std(c_data(:,3), ...
    'omitmissing')/sqrt(14))] ...
    )

aov_a = anova(a_data);
pval_a = aov_a.stats{1,5};

disp(['One-way ANOVA comparing asymptote between groups: F(' num2str(aov_a.stats{1,2}) ',' num2str(aov_a.stats{2,2}) ') = ' num2str(aov_a.stats{1,4}) ', p = ' num2str(pval_a) '.']);

aov_b = anova(b_data); 
pval_b = aov_b.stats{1,5}; 

disp(['One-way ANOVA comparing magnitude between groups: F(' num2str(aov_b.stats{1,2}) ',' num2str(aov_b.stats{2,2}) ') = ' num2str(aov_b.stats{1,4}) ', p = ' num2str(pval_b) '.']);

aov_c = anova(c_data); 
pval_c = aov_c.stats{1,5}; 

disp(['One-way ANOVA comparing rate between groups: F(' num2str(aov_c.stats{1,2}) ',' num2str(aov_c.stats{2,2}) ') = ' num2str(aov_c.stats{1,4}) ', p = ' num2str(pval_c) '.']);
...
disp(['Rest retention: ', ...
    num2str(mean(diff_perf(:,1),'omitmissing')), ...
    ' ± ', ...
    num2str(std(diff_perf(:,1), ...
    'omitmissing')/sqrt(15))] ...
    )

disp(['ENC retention: ', ...
    num2str(mean(diff_perf(:,2),'omitmissing')), ...
    ' ± ', ...
    num2str(std(diff_perf(:,2), ...
    'omitmissing')/sqrt(15))] ...
    )

disp(['SEM retention: ', ...
    num2str(mean(diff_perf(:,3),'omitmissing')), ...
    ' ± ', num2str(std(diff_perf(:,3), ...
    'omitmissing')/sqrt(14))] ...
    )
...
title(' Effect of group on retention (# correct sequences) '); 
...
% Save figure
savefig([fig_dir, '\Figure_2D.fig']);
exportgraphics(gca, [fig_dir, '\Figure_2D.tiff'], 'Resolution', 300)
