...
% Notes: 
% 1) Output is the effect size generated from a Group by Block ANOVA on
% MOGS (first 11 trials). 
% 2) Script generates number of subjects required to test whether there
...
requiredSampleSize = ceil( ...
    (norminv(0.95) + norminv(0.85))^2 * (std(tbl.Microoffline, 'omitmissing')^2) / effectSize^2);
