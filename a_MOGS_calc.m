...
% This script organizes the data for all analyses. 
% Requirements: Access to "MOGS_data.txt" File.
%               Access to an empty folder called "_processed_data".
%
...
out_dir = [home_dir, '/_processed_data']; % output data folder
...
% Read in data
MOGS_data = readtable('MOGS_data.txt','VariableNamingRule', 'preserve');

disp(' - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ');
disp('         Collecting and organizing performance metrics         ');
disp(' - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ');

% Dummy matrix with participant, trials, and groups as dimensions
% Seq_score represents the number of completed correct sequences, including
% those that are partially correct by the end of the trial.
seq_score = zeros(15,33,3); 

% Group loop
mem_perf_sorted = zeros(24,2,15,3);
...
for grp=1:length(seq_score(1,1,:)) 
    ...
    for subject = 1:size(subs,2)
        ...
        % Organize ESL data by trial, response, computer time, reaction
        % time, accuracy, and episodic memory recall
        ...
        RetSlide_ACC=alldata.("RetSlide.ACC"); % Pull Episodic memory test accuracy data
        Retreival=alldata.("Retreival"); % Pull Retrieval trial items

        % Sort episodic memory accuracy data by trial experience (1-24).
        if grp==1
            mem_perf_sorted(1:24,2,subject,grp) = 0; 
        else
            mem_perf(:,1) = Retreival(~isnan(Retreival)); 
            mem_perf(:,2) = RetSlide_ACC(~isnan(RetSlide_ACC));
            [sorted_trial, sort_idx] = sort(mem_perf(:,1));
            mem_perf_sorted(:,:,subject,grp) = mem_perf(sort_idx, :);
        end
    ...
    clearvars -except grp microoffline microonline ITI home_dir mem_perf mem_perf_sorted home_dir out_dir seq_score seq_ccount MOGS_data
