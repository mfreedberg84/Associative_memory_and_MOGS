...
out_dir = '';
bp = 11; % Indicate break point

% Tests: 
% 1) Average MOGS for first X trials against zero.
% 2) Average MOnGS for the first X trials against zero.
% 3) Average MOGS vs. MOnGS for first X trials
...
disp(['Mean MOGS = ', ...
    num2str(mean(rest_mogs(1:bp))), ...
    ', SEM = ', ...
    num2str(std(rest_mogs(1:bp))/sqrt(length(rest_mogs))), ...
    '.'] ...
    )
...
disp(['Mean MOnGS = ', ...
    num2str(mean(rest_mons(1:bp))), ...
    ', SEM = ', ...
    num2str(std(rest_mons(1:bp))/sqrt(length(rest_mons))), ...
    '.'] ...
    )
...
sigma0 = 0.4117;        % Estimated standard deviation under the null hypothesis
...
pwrout = sampsizepwr('t',[mu0 sigma0],mu1,[],nn,'Alpha',0.05);
