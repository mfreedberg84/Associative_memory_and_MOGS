clear; clc;
%% Analysis 6: Break point analysis MOnGS
%
% Requirements: Access to files generated from "a_MOGS_MOnGS_calc.m".
%
% indicate where your data output from a_MOGS_MOnGS_calc.m wen 
% and where the figures should go.
out_dir = '';
fig_dir = '';

% Tests: 
% 1) LME contrasting MOnGS across groups with trial number as covariate and
...
disp(' - - - - - - - - - - - - - - - - - - - - - - - - -');
disp('              Break Point Analysis MOnGS          ');
disp(' - - - - - - - - - - - - - - - - - - - - - - - - -');
    ...
    for subject = 1:15
        for bp = 1:24 % all possible breakpoints
            bp_groups(subject,bp,grp)= mean( ...
                microonline(1:bp,subject,grp),1, 'omitmissing' ...
                );
        end
    end
end
...
bp_data = pagetranspose(bp_groups(:,1:24,:));
...
Groups = [1,2,3];
Group_iv = repelem(Groups, 1, 360)'; % Repeats each element 3 times

% Create subject variable for ANOVAn for GROUP
subs = 1:45;
subs_iv = repelem(subs,1, 24)';
...
tbl = array2table(tbl, 'VariableNames', ...
    {'Group', 'Subject', 'Trial_numb', 'Microonline' ...
    });

% Run LME Contrasting BLOCK and GROUP with a BLOCK COVARIATE
lme = fitlme(tbl,'Microonline~Group+Trial_numb+(1|Subject)');
...
disp(' - - - - - - - - - - - - - - - - - - - - - - - - -');
disp('        Finished MOGS Break Point Analysis        ');
disp(' - - - - - - - - - - - - - - - - - - - - - - - - -');
