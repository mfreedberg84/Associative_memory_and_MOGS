clear; clc;
%% Analysis 1: Effects of each group on performance and retention 
%
% Requirements: Access files generated from "a_MOGS_calc.m".
%
...
%    "b2_curvefit_mogs.m," "b3_exponentialfit.m," "b4_exponential_ls.m," 
%    and "b5_exponential.ms." These functions will determine the
...
load([load_dir, '/ITI.mat'])
load([load_dir, '/Number_of_correct_sequences.mat'])
...
load([load_dir, '/Correct_sequence_data.txt'])
c2_curvefit_mogs;  
writematrix(output,[out_dir, '/' outfile])
cf_output = readmatrix([load_dir, '/Correct_sequence_data_output.txt']);
...
clearvars -except out_dir a_data b_data back_dir c_data data diff_perf home_dir ITI mean_perf mem_perf mem_perf_sorted microonline microoffline seq_score numb_corr_performance numb_corr_retention seq_score  
disp('press any button to continue...');
pause; close;
disp(repmat(char(8), 1, 32));
