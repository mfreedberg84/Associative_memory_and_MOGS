...
out_dir = [root_dir '\processed_data']; % output data folder
mkdir(out_dir);
...
%    performance of correct sequences will be recorded in keypresses per
%    second (KPS) --> ITI.
...
%    be patially added on to the running total, such that completing
...
%    In the second method, any correct partial sequence at the end of the trial 
%    counts as a full sequence ("seq_count").  
% 5) Online gains ("microonline") are calculated for all 33 
...
%    on trial X+1 and the last correct sequence on trial X 
...
for grp=1:length(seq_score(1,1,:)) 
    ...
    for subject = 1:size(subs,2)
        ...
        alldata = MOGS_data( ...
            MOGS_data.Subject == str2double(subs{subject}),:);
        ...
        for trl=1:33 
            ...
            while (tapping <= tap_trl)
                ...
                if(tapping<=tap_trl-4)
                    ...
                    if (resp_data(tap,1)==4 && resp_data(tap+1,1)==1 && ...
                            resp_data(tap+2,1)==3 && ...
                            resp_data(tap+3,1)==2 && ...
                            resp_data(tap+4,1)==4 ...
                            ) 
                        ...
                        seq_score(subject, trl,grp) = seq_score( ...
                            subject, trl,grp)+1; 
                        
                        % % Calculate keypresses per second (time between
                        % first button press and last in seconds)
                        ITI(seq_count,trl,subject,grp)= 1/(((iti_data( ...
                            tap+4,1)-iti_data(tap,1))/4)/1000); 
                ...
                % If it's one of the final five taps in the trial...
                    ...
                    % increment the count of incomplete trials 
                    incomplete_seq(trl,subject,grp) = incomplete_seq(trl,subject,grp)+1;
                    ...
                    switch tapping_left 
                        ...
                        case 4 
                            ...
                            if (resp_data(tap,1)==4 && resp_data(tap+1,1)==1 && ...
                                ...
                                % those four keypresses
                                 ITI(seq_count,trl,subject,grp) = ( ...
                                     1/(((iti_data(tap+3,1)-iti_data(tap,1))/3)/1000) ...
                                     );
                        ...
                        case 3 
                            ...
                            if (resp_data(tap,1)==4 && resp_data(tap+1,1)==1 && ...
                                ...
                                % those 3 keypresses
                                ITI(seq_count,trl,subject,grp) = ( ...
                                    1/(((iti_data(tap+2,1)-iti_data(tap,1))/2)/1000) ...
                                    );
                        ...
                        case 2 
                            ...
                            if (resp_data(tap,1)==4 && resp_data(tap+1,1)==1) % If the next two responses are correct
                                ...
                                ITI(seq_count,trl,subject,grp) = ( ...
                                    1/(((iti_data(tap+1,1)-iti_data(tap,1))/1)/1000) ...
                                    );
        ...
        for trl=1:33
            ...
            if ITI(1,trl,subject,grp) ~= 0
                ...
                disp(['error in first mogs. 0 in ITI trial = ', ...
                    num2str(trl), ...
                    ', sub=', ...
                    num2str(subject)]);
            end
        ...
        for trl=1:33
            ...
            % as NaNs
            microonline(trl,subject,grp)=last_cor_seq( ...
                trl,subject,grp)-first_cor_seq(trl,subject,grp ...
                ); 
        end
        ...
        for trl=1:32
            ...
            microoffline(trl,subject,grp)=first_cor_seq( ...
                trl+1,subject,grp) - last_cor_seq(trl,subject,grp ...
                );
        end
    end % Subject
    clearvars -except grp microoffline microonline ITI mem_perf mem_perf_sorted home_dir out_dir root_dir seq_score seq_ccount MOGS_data incomplete_seq
