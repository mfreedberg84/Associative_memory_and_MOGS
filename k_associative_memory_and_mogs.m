...
out_dir = 'G:\My Drive\DataScience\UT_Austin\MOGS\processed_data'; %root_dir;
fig_dir = 'C:\Users\mfree\OneDrive - University of Iowa\UIowa\_papers\MOGS_Hipp\_figures\_figure3';


% Notes:
%   1) This script examines the association between associative memory
...
scatter(tmp_sem(:,1), tmp_sem(:,2),150, ...
    'MarkerEdgeColor',[0 0 0], ...
...
z1 = atanh(ENC_MOGS_r(1,2));
z2 = atanh(SEM_MOGS_r(1,2));
...
% --- Display results ---
fprintf('Correlation 1 (n=%d): r = %.4f, z = %.4f\n', n1, ENC_MOGS_r(1,2), z1);
fprintf('Correlation 2 (n=%d): r = %.4f, z = %.4f\n', n2, SEM_MOGS_r(1,2), z2);
fprintf('Z-statistic for difference: %.4f\n', z_stat);
fprintf('p-value: %.4f\n', p_value);

if p_value < 0.05
    disp('Result: The ENC and SEM correlations are significantly different for MOGS');
else
    disp('Result: The ENC and SEM correlations are NOT significantly different for MOGS');
end
