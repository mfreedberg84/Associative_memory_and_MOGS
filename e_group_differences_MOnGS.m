...
% where the figures should go.
out_dir = ''; %root_dir;
fig_dir = '';

bp = 11; % indicate the break point
...
% Prepare the table for LME analysis
tbl_rest = array2table(tbl_rest, 'VariableNames', ...
    {'Subject', ...
    'Block', ...
    'Microonline'} ...
    );
...
% Prepare the table for LME analysis
tbl_enc = array2table(tbl_enc, 'VariableNames', ...
    {'Subject', ...
    'Block', ...
    'Microonline'} ...
    );
...
% Prepare the table for LME analysis
tbl_sem = array2table(tbl_sem, 'VariableNames', ...
    {'Subject', ...
    'Block', ...
    'Microonline'} ...
    );
...
% Calculate MOnGs for all intertrial periods
microonline24 = microonline(1:24,:, :);

plot_mogs_m = zeros(24,3);
plot_mogs_s = zeros(24,3);
for i=1:3
    if i == 3
        % Remove column 10
        tmp_sem = [microonline24(:,1:9,3), microonline24(:,11:15,3)];
        plot_mogs_m(:,3) = mean(tmp_sem,2,'omitmissing');
        plot_mogs_s(:,3) = std(tmp_sem,0,2,'omitmissing')/sqrt(14);
    else
        plot_mogs_m(:,i) = mean(microonline24(:,:,i),2,'omitmissing');
        plot_mogs_s(:,i) = std(microonline24(:,:,i),0,2,'omitmissing')/sqrt(15);
    end
end

disp(['Rest MOnGS: ', ...
    num2str(mean(plot_mogs_m(:,1),'omitmissing')), ...
    ' ± ', ...
    num2str(std(plot_mogs_m(:,1), ...
    'omitmissing')/sqrt(15))] ...
    )

disp(['ENC MOnGS: ', ...
    num2str(mean(plot_mogs_m(:,2),'omitmissing')), ...
    ' ± ', ...
    num2str(std(plot_mogs_m(:,2), ...
    'omitmissing')/sqrt(15))])

disp(['SEM MOnGS: ', ...
    num2str(mean(plot_mogs_m(:,3),'omitmissing')), ...
    ' ± ', num2str(std(plot_mogs_m(:,3), ...
    'omitmissing')/sqrt(14))] ...
    )
...
title(['Group differences in MOnGS'], fontsize = 24);
...
% Save figure
savefig([fig_dir, '\Figure_2B.fig']);
exportgraphics(gca, [fig_dir, '\Figure_2B.tiff'], 'Resolution', 300)
